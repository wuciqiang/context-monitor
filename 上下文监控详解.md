# 上下文监控系统
# Context Monitoring System

> 半自动化的 Claude Code 上下文使用率监控方案

---

## 📊 系统概述

本系统通过 **Hook + MCP Server + CLAUDE.md 指令** 的组合，实现对 Claude Code 会话上下文使用率的半自动监控。

### 核心组件

1. **SessionStart Hook** - 捕获会话信息（transcript 路径）
2. **Context Monitor MCP Server** - 提供监控工具
3. **CLAUDE.md 指令** - 指导 Claude 定期检查和响应

### 工作流程

```
会话开始
  ↓
SessionStart Hook 捕获 transcript_path
  ↓
写入 /tmp/claude-session-info.json
  ↓
Claude 每 5-10 个工具调用后
  ↓
调用 check_context_usage 工具
  ↓
MCP Server 读取 transcript 文件大小
  ↓
计算使用率并返回状态
  ↓
Claude 根据使用率采取行动：
  - < 50%: 继续工作
  - 50-70%: 注意使用率
  - 70-85%: 准备保存状态
  - > 85%: 立即保存并提示用户 /clear
```

---

## 🚀 安装配置

### 1. 文件结构

确保以下文件存在：

```
.claude/
├── hooks/
│   └── capture-session-info.sh       # SessionStart hook
├── mcp-servers/
│   └── context-monitor/
│       └── server.py                  # MCP server
├── settings.local.json                # Hook 和 MCP 配置
└── CONTEXT_MONITORING.md              # 本文档

CLAUDE.md                              # 包含上下文管理指令
```

### 2. 赋予执行权限

```bash
chmod +x .claude/hooks/capture-session-info.sh
chmod +x .claude/mcp-servers/context-monitor/server.py
```

### 3. 验证配置

检查 `.claude/settings.local.json`：

```json
{
  "hooks": {
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash .claude/hooks/capture-session-info.sh",
            "timeout": 5
          }
        ]
      }
    ]
  },
  "mcpServers": {
    "context-monitor": {
      "command": "python3",
      "args": [".claude/mcp-servers/context-monitor/server.py"],
      "env": {}
    }
  }
}
```

---

## 🔧 使用方法

### 自动启动

当你启动 Claude Code 会话时：
1. SessionStart hook 自动运行
2. 会话信息被捕获到 `/tmp/claude-session-info.json`
3. Context Monitor MCP Server 自动加载
4. Claude 会看到提示："📊 Context monitoring active"

### 手动检查

在对话中，你可以随时要求 Claude 检查：

```
请检查当前上下文使用率
```

Claude 会调用 `check_context_usage` 工具并报告结果。

### Claude 的自动行为

根据 CLAUDE.md 中的指令，Claude 会：
- 每 5-10 个工具调用后自动检查
- 根据使用率采取相应行动
- 在高使用率时主动保存状态并提示你执行 `/clear`

---

## 📈 MCP 工具说明

### 1. check_context_usage

**功能**：检查当前会话的上下文使用率

**输入**：无参数

**输出**：
```json
{
  "session_id": "abc123",
  "transcript_path": "/path/to/transcript.jsonl",
  "file_size_mb": 3.45,
  "estimated_tokens": 45000,
  "usage_percent": 22.5,
  "max_tokens": 200000,
  "status": "✅ SAFE",
  "recommendation": "Context usage is healthy. Continue working normally.",
  "timestamp": "2025-12-16T10:30:00Z"
}
```

**状态级别**：
- `✅ SAFE` (< 50%)
- `⚠️ WARNING` (50-70%)
- `🔴 HIGH` (70-85%)
- `🚨 CRITICAL` (> 85%)

### 2. save_session_state

**功能**：保存会话状态到 `.claude/state/current-session.md`

**输入**：
```json
{
  "content": "当前进度描述",
  "next_steps": "下一步要做什么"
}
```

**输出**：
```json
{
  "success": true,
  "state_file": ".claude/state/current-session.md",
  "message": "Session state saved successfully"
}
```

---

## ⚠️ 重要说明

### 限制

1. **无法自动执行 /clear** - 这是 Claude Code 的架构限制，必须由用户手动执行
2. **Token 估算不精确** - 使用文件大小估算，可能有 ±10% 的误差
3. **需要 Python 3** - MCP server 需要 Python 3 环境
4. **macOS/Linux 优先** - Windows 可能需要调整脚本

### 最佳实践

1. **信任 Claude 的判断** - 当 Claude 提示保存状态时，立即执行
2. **定期检查** - 在长时间会话中，主动要求 Claude 检查使用率
3. **及时清除** - 不要等到 100% 才清除，70-80% 就应该考虑
4. **保存完整状态** - 确保 `save_session_state` 包含足够的信息以便恢复

---

## 🐛 故障排查

### 问题：Hook 没有运行

**检查**：
```bash
# 查看 hook 是否有执行权限
ls -l .claude/hooks/capture-session-info.sh

# 手动测试 hook
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","cwd":"."}' | bash .claude/hooks/capture-session-info.sh
```

### 问题：MCP Server 无法启动

**检查**：
```bash
# 测试 Python 环境
python3 --version

# 手动运行 server
python3 .claude/mcp-servers/context-monitor/server.py
```

### 问题：check_context_usage 返回错误

**检查**：
```bash
# 查看会话信息文件
cat /tmp/claude-session-info.json

# 检查 transcript 文件是否存在
ls -lh ~/.claude/transcripts/
```

### 问题：使用率计算不准确

**说明**：
- 使用文件大小估算 token 数量（1 token ≈ 4 bytes）
- 这是粗略估算，实际可能有偏差
- 建议在 60-70% 时就开始准备清除

---

## 🔄 恢复会话

当你执行 `/clear` 后，使用以下提示词恢复：

```
请阅读 .claude/state/current-session.md 了解之前的进度，然后继续工作。
```

Claude 会：
1. 读取状态文件
2. 了解之前的进度
3. 继续未完成的任务

---

## 📚 技术细节

### Token 估算公式

```python
estimated_tokens = file_size_bytes / 4
usage_percent = (estimated_tokens / 200000) * 100
```

### 为什么是 200,000 tokens？

- Claude Sonnet 4.5 的上下文窗口是 200k tokens
- 实际可用可能略少（系统提示词占用）
- 建议在 70-80% 时清除以保持安全边际

### 会话信息文件格式

```json
{
  "session_id": "唯一会话 ID",
  "transcript_path": "transcript 文件的绝对路径",
  "cwd": "当前工作目录",
  "timestamp": "ISO 8601 时间戳"
}
```

---

## 🎯 未来改进

可能的增强方向：

1. **更精确的 token 计算** - 使用 tiktoken 库精确计算
2. **可视化仪表板** - Web UI 显示实时使用率
3. **自动备份** - 定期自动保存状态
4. **多会话支持** - 跟踪多个并行会话
5. **历史分析** - 分析历史使用模式

---

## 📞 支持

如有问题或建议，请：
1. 检查本文档的故障排查部分
2. 查看 `.claude/logs/` 中的日志
3. 在项目中创建 Issue

---

**版本**: 1.0.0
**最后更新**: 2025-12-16
**维护者**: Project Team
